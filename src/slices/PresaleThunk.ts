...
import { abi as Airdrop } from "../abi/Airdrop.json";
...
export const changeDeposit = createAsyncThunk(
  ...
  async ({ action, value, provider, address, networkID }: IActionValueAsyncThunk, { dispatch }) => {
    ...
    const signer = provider.getSigner();
    const airdrop = new ethers.Contract(addresses[networkID].AIRDROP_ADDRESS as string, Airdrop, signer);
    ...
    try {
      uaData.type = "claim";
      depositTx = await airdrop.claim();
      const pendingTxnType = "claiming";
      uaData.txHash = depositTx.hash;
      dispatch(fetchPendingTxns({ txnHash: depositTx.hash, text: "claiming...", type: pendingTxnType }));
